...
def amsdefault(amsin):
    ...
    if amsin is None:
        if "AMSHOME" in os.environ:
            if os.path.isdir(os.environ["AMSHOME"]):
                return os.environ["AMSHOME"]
        else:
            if os.path.isdir(AMSDEFAULT):
                return AMSDEFAULT
    else:
...
def amsparser(parser):
    ...
    # Add options
    parser.add_argument(dest="inp", nargs="+", metavar="INPUT", help="Input file(s)")
    ...
    parser.add_argument(
        "-p", "--nproc", metavar="NSCM", type=int, dest="nproc", default=1, help="Set number of processors"
    )
    # parser.add_argument('-t', '--tmp', metavar='SCM_TMPDIR',
    #     dest='tmp', action='store', default=None,
    #     help='Set scratch directory')
    parser.add_argument(
        "-mail", dest="mail", action="store_true", default=False, help="Send the user an email at the end of the script"
    )
    parser.add_argument("-to", "--to", nargs=1, dest="to", action="store", default=None, help="Specify mail recepients")
    ...
    for num, inp in enumerate(opts.inp, start=1):
        ...
        if not opts.dry:
            try:
                ...
                bashrun(amscmd, env=os.environ, vrb=opts.vrb)
                # Email results
                if opts.mail or opts.to:
                    try:
                        ...
                        if opts.vrb:
                            argmail.append("-v")
                        if opts.to:
                            argmail.extend(["-to", f"{opts.to}"])
                        mail.main(argmail)
                    except Exception:
                        print("WARNING: Failed to send mail")
            except Exception:
