...
GITPREFS = {"dir": "dati-", "fil": "dpc-covid19-ita-"}
GITNAMS = {"nazione": "andamento-nazionale", "regione": "regioni", "provincia": "province"}
GITHEDS = {
    "nazione": "stato",
...
def parseopt():
    ...
    # Mandatory arguments
    parser.add_argument(
        "luogo", type=str, nargs="?", default="pisa", action="store", help="Luogo di cui cercare i dati"
    )
    # Optional arguments
    parser.add_argument(
        "-media", "-m", "-average", action="store_true", dest="media", default=False, help="Media sulla settimana"
    )
    parser.add_argument("-log", action="store_true", dest="log", default=False, help="Plotta scala logaritmica")
    parser.add_argument(
        "-diff",
        "-nuovi",
        "-delta",
    ...
    parser.add_argument("-s", "--search", type=str, default=TOSEARCH, dest="src", action="store", help="Cosa cercare")
    parser.add_argument("-t", "--type", choices=GITNAMS.keys(), dest="tipo", help=argparse.SUPPRESS)
    parser.add_argument("-v", "--iprint", dest="iprt", action="count", default=0, help="Livello di stampa")
...
def fileurl(tipoluogo, desinenza):
    ...
    subpath = (
        GITPREFS["dir"] + GITNAMS[tipoluogo] + "/" + GITPREFS["fil"] + GITNAMS[tipoluogo] + "-" + desinenza + ".csv"
    )
    # Cerca in locale
...
def discretize(z, zmax, zmin, LZ, action=None):
    ...
    if action == "log":
        # print(f'{zmin} {zmax} {z}')
        if zmin < 0.0 or zmax < 0.0 or z < 0.0:
            errore("Log scale with negative values")
        if (z - zmin) <= TEST0:
            zrel = 0.0
        else:
...
def main():
    ...
    gstart = 0
    if opts.media:
        casi = [int(sum(casi[i : i + 7]) / 7.0) for i in range(len(casi) - 6)]
        nuovicasi = [int(sum(nuovicasi[i : i + 7]) / 7.0) for i in range(len(nuovicasi) - 6)]
        gstart = 6
