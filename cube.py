...
def cubeparse():
    # CREATE PARSER
    parser = argparse.ArgumentParser(prog=PROGNAME, description="Helps with building cubegen command")
    # MANDATORY ARGUMENTS
    parser.add_argument("fchk", help="Formatted checkpoint file name")
    parser.add_argument("kind", help="Type of cube to be generated")
    # OPTIONAL ARGUMENTS
    parser.add_argument("-n", "--nproc", dest="nproc", default=0, help="Set number of processors")
    parser.add_argument("-o", "--cubefile", dest="cub", help="Set cube file name")
    parser.add_argument(
        "-p", "--npts", dest="npts", default=0, metavar="NPTS", type=int_or_str, help="Set number of points"
    )
    parser.add_argument("-v", "--verbose", dest="vrb", action="count", default=0, help="Set printing level")
    parser.add_argument("-f", "--format", dest="cbfmt", default="h", help="Set format")
    parser.add_argument("-w", "--wrkdir", dest="wrkdir", metavar="WORKDIR", default="", help="Work directory path")
    parser.add_argument("-q2", "--cubefile2", dest="cub2", default="junk.cub", help=argparse.SUPPRESS)
    parser.add_argument("-a", "--add", dest="bonus", default="0", help="Additional custom integer flag")
    parser.add_argument("-d", "--denfil", dest="denfil", help="Set density file name")
    #   OPTION PARSING
