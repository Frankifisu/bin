...
DEFAULT_PATHS = {"hpc_inifile": "/home/{}/{}".format(JULIEN, HPCINIFILE), "hpc_modpath": "/home/{}/lib".format(JULIEN)}
...
""".format(
    julien=JULIEN, workdir=WORKINGDIR
)
...
def build_parser():
    ...
    """
    parser = argparse.ArgumentParser(prog=PROGNAME, formatter_class=argparse.RawTextHelpFormatter)
    #  MANDATORY ARGUMENTS
    # ---------------------
    ...
    queue.add_argument("-j", "--job", dest="job", help="Sets the job name. (NOTE: PBS truncates after 15 characaters")
    queue.add_argument("-m", "--mail", dest="mail", action="store_true", help="Sends notification emails")
    queue.add_argument("--mailto", dest="mailto", help="Sends notification emails")
    queue.add_argument(
        "-M",
        "--mach",
        dest="mach",
    ...
    queue.add_argument("--multi", choices=("parallel", "serial"), help="Runs multiple jobs in a single submission")
    queue.add_argument("--node", dest="node", type=int, help="Name of a specific node (ex: curie01)")
    queue.add_argument("--group", dest="group", type=str, help="User group")
    queue.add_argument(
        "-p", "--project", dest="project", default="account_test", help="Defines the project to run the calculation"
    )
    queue.add_argument(
        "-P", "--print", dest="prtinfo", action="store_true", help="Print information about the submission process"
    )
    queue.add_argument(
        "-q",
        "--queue",
...
WARNING: The consequence will be a loss of these outputs""",
    ...
    expert.add_argument(
        "--cpto", dest="cpto", nargs="+", help="Files to be copied to the local scratch (dumb copy, no check)"
    )
    expert.add_argument(
        "--cpfrom", dest="cpfrom", nargs="+", help="Files to be copied from the local scratch (dumb copy, no check)"
    )
    expert.add_argument(
        "--nojob", dest="nojob", action="store_true", help="Do not run job. Simply generate the input sequence."
    )
    expert.add_argument(
        "-X",
        "--expert",
...
""",
    ...
    gaussian.add_argument("-c", "--chk", dest="gxxchk", metavar="CHK_FILENAME", help="Sets the checkpoint filename")
    gaussian.add_argument(
        ...
        metavar="GAUSSIAN",
        default="g16c01",
        help="{}\n{}".format("Sets the path to the Gaussian executables.", HELP_GXX),
    )
    gaussian.add_argument(
        "-i",
        "--ignore",
...
""",
    ...
    gaussian.add_argument("-o", "--out", dest="gxxlog", metavar="LOG_FILENAME", help="Sets the output filename")
    gaussian.add_argument(
        "-r",
        "--rwf",
        dest="gxxrwf",
        metavar="RWF_FILENAME",
        help="""\
...
""",
    ...
    gaussian.add_argument(
        ...
        metavar="PEMBED",
        default="1",
        help="Model parameters for polarizable embedding",
    )
...
def set_workingpath(gspec: str, gxx_arch: str, owner: typing.Optional[str] = None) -> typing.Union[str, bool]:
...
def set_gxxroot(gspec: str, gxx_arch: str) -> typing.Tuple[str, typing.Union[str, None]]:
...
def set_gxx_env(gxxroot: str) -> typing.List[str]:
    ...
    """
    dirlist = [os.path.join(gxxroot, f) for f in ["bsd", "local", "extras", ""]]
    ...
    if "PATH" in os.environ:
        os.environ["PATH"] = os.pathsep.join([GAUSS_EXEDIR, os.environ["PATH"]])
    else:
        os.environ["PATH"] = GAUSS_EXEDIR
    if "LD_LIBRARY_PATH" in os.environ:
        os.environ["LD_LIBRARY_PATH"] = os.pathsep.join([GAUSS_EXEDIR, os.environ["LD_LIBRARY_PATH"]])
    else:
...
def check_gjf(
    ...
    def process_route(route: str) -> typing.Union[bool, bool, bool, bool, bool, typing.List[str]]:
        ...
        if key_718o.search(route):
            ...
            opt718 = True
            if not key_718:
                s = "WARNING: It is not yet possible to run FCHT " + "calculations with ReadFCHT only"
                print(s)
        ...
        with open(gjf_ref, "r") as fobjr:
            for line in fobjr:
                ...
                if not line_lo:
                    fobjw.write(line)
                    if inroute:
                        usepem[-1], use717[-1], opt717[-1], use718[-1], opt718[-1], dat = process_route(route[-1])
                        if dat:
                            ops_copy.extend(dat)
                        inroute = False
                    continue
...
def get_queue_data(
    full_queue: str,
) -> typing.Tuple[str, hpc.NodeFamily, int, typing.Union[str, None]]:
    ...
    elif family.cpu_limits["hard"] is not None and nprocs > family.cpu_limits["hard"]:
...
"""
    ...
    if opts.node is not None:
        if nodeid is not None and nodeid != opts.node:
            msg = "ERROR: Different nodes selected through virtual queue " + "and option"
            print(msg)
            sys.exit
        else:
            nodeid = opts.node
    ...
    if set(["p", "proc", "a", "all"]) & set(opts.gxxl0K):
        ...
        if nprocs == 0:
            msg = "ERROR: Too many parallel jobs for the number of " + "processing units"
            print(msg)
            sys.exit()
    if set(["m", "mem", "a", "all"]) & set(opts.gxxl0K):
        ...
        if nprocs is None:
            ...
            factor = min(1.0, nprocs / qnode.nprocs(all=USE_LOGICAL_CORE))
    ...
    for index, infile in enumerate(opts.infile):
        ...
        os.chdir(rootdir)
        if not opts.expert:
            dat_P, dat_M, data = check_gjf(ginfile, gjf_new, nprocs, mem, chkfile, rwffile, rootdir)
    ...
    if not opts.expert:
        if full_P > qnode.nprocs(all=USE_LOGICAL_CORE):
            ...
            sys.exit()
        elif qnode.cpu_limits["hard"] is not None and full_P > qnode.cpu_limits["hard"]:
            print("ERROR: number of processors exceeds hard limit")
            sys.exit()
        if qnode.mem_limits["hard"] is not None and full_M > qnode.mem_limits["hard"]:
            print("ERROR: Requested memory exceeds hard limit")
            sys.exit()
        nprocs = full_P
        mem = hpc.bytes_units(full_M, 0, False, "g")
    if qnode.cpu_limits["soft"] is not None and nprocs > qnode.cpu_limits["soft"]:
        print("NOTE: Number of processors exceeds soft limit.")
    if qnode.mem_limits["soft"] is not None and hpc.convert_storage(mem) > qnode.mem_limits["soft"]:
...
echo "----------------------------------------"
  """.format(
        scrdir=tmpdir, input=", ".join(opts.infile)
    )
    ...
    for index, gjf_file in enumerate(gjf_files):
        ...
        rootdir = rootdirs[index]
        pbs_cmds += fmt.format(gexe=gxx, gargs=gxx_args.strip(), gin=gjf_file, gout=os.path.join(rootdir, log_file))
    if multi_gjf and opts.multi == "parallel":
        pbs_cmds += "wait\n"
    ...
    if opts.mail:
        ...
        if opts.mailto:  # Full recipient address given
            ...
            fmt += " -M {user}@{server}"
        qsub_args.append(fmt.format(addr=opts.mailto, user=USERNAME, server=EMAILSERVER))
    ...
    qsub_args.append(fmt.format(ncpus=ncpus, mem=mem, family=qnode.queue_name, node=nodeid))
    # Queue name
