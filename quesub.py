...
def queueparser(parser):
    ...
    parser.add_argument("-p", "--nproc", metavar="PROC", dest="ppn", default=28, help="Set number of processors")
    parser.add_argument("-mpi", dest="mpi", default=False, action="store_true", help="Request MPI parallelization")
    parser.add_argument("-v", "--verbose", dest="vrb", action="count", default=0, help="Set printing level")
    parser.add_argument(
        "-h", "--help", dest="hlp", action="store_true", default=False, help="Print help message and exit"
    )
    return parser
...
def parseopt(args=None):
    ...
    helparser = argparse.ArgumentParser(
        prog=PROGNAME,
